...
func (x *ExampleMsg) MarshalProtoJSON(s *json.MarshalState) {
	...
	s.WriteObjectStart()
	var wroteField bool
	if x.ExampleField != "" || s.HasField("example_field") {
		s.WriteMoreIf(&wroteField)
		s.WriteObjectField("example_field")
		s.WriteString(x.ExampleField)
	}
	if x.OtherMsg != nil || s.HasField("other_msg") {
		s.WriteMoreIf(&wroteField)
		s.WriteObjectField("other_msg")
		x.OtherMsg.MarshalProtoJSON(s.WithField("other_msg"))
	}
	s.WriteObjectEnd()
}
...
func (m *ExampleMsg) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		if wireType == 4 {
			return errors.Errorf("proto: ExampleMsg: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return errors.Errorf("proto: ExampleMsg: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 2 {
		...
		case 2:
			if wireType != 2 {
				return errors.Errorf("proto: wrong wireType = %d for field OtherMsg", wireType)
			}
			var msglen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
