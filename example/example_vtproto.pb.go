...
func (m *ExampleMsg) CloneVT() *ExampleMsg {
	...
	if rhs := m.OtherMsg; rhs != nil {
		if vtpb, ok := interface{}(rhs).(interface{ CloneVT() *other.OtherMsg }); ok {
			r.OtherMsg = vtpb.CloneVT()
		} else {
			r.OtherMsg = proto.Clone(rhs).(*other.OtherMsg)
		}
	}
...
func (this *ExampleMsg) EqualVT(that *ExampleMsg) bool {
	...
	if equal, ok := interface{}(this.OtherMsg).(interface{ EqualVT(*other.OtherMsg) bool }); ok {
		if !equal.EqualVT(that.OtherMsg) {
			return false
		}
	} else if !proto.Equal(this.OtherMsg, that.OtherMsg) {
		return false
	}
...
func (m *ExampleMsg) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if m.OtherMsg != nil {
		if vtmsg, ok := interface{}(m.OtherMsg).(interface {
			...
			encoded, err := proto.Marshal(m.OtherMsg)
			if err != nil {
				return 0, err
			}
			i -= len(encoded)
...
func (m *ExampleMsg) SizeVT() (n int) {
	...
	if m.OtherMsg != nil {
		if size, ok := interface{}(m.OtherMsg).(interface {
			SizeVT() int
		}); ok {
			l = size.SizeVT()
		} else {
			l = proto.Size(m.OtherMsg)
		}
		n += 1 + l + sov(uint64(l))
	}
...
func (m *ExampleMsg) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 2:
			...
			if unmarshal, ok := interface{}(m.OtherMsg).(interface {
				...
				if err := proto.Unmarshal(dAtA[iNdEx:postIndex], m.OtherMsg); err != nil {
					return err
				}
			}
			iNdEx = postIndex
